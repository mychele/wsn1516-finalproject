...
Given a certain $K$, the sender reads a portion of the file and creates $N$ encoded packets (as \texttt{NCpacket} objects). Then it sends them as UDP datagrams and starts waiting for an acknowledgment from the receiver. When an ACK is received, the sender checks for the number $P$ of packets still needed. If this is 0, it continues with the next block to be sent, otherwise it sends $P$ new encoded packets. In the payload of the first transmitted packet the sender reserves 4 byte to communicate to the receiver the file size, so that this terminates correctly once the whole file is received. 
