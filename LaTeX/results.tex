...
In Fig. \ref{fig:pm1} we show the results we obtain by enabling the PER estimation mode. In this setup, the sender tries to estimate the PER, in order to improve system performance. We see that effectively we have quite a significant increase in goodput, while the efficiency remains approximately the same (except for the case $PER=0$, which gives some estimation troubles, probably due to convergence problems of estimators). Therefore we have that our PER estimation method proves to be quite useful, even though performance doesn't change dramatically. This is due to the fact that the number of blocks (i.e. $7$ for $K=3500$) used in an LT setup transmission is quite small and therefore estimation is not very accurate.
