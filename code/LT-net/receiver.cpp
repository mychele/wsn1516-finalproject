...
int main(int argc, char *argv[])
{
    ...
    double total_time_decoding_attempt = 0;
    int num_decoding_attempt = 0;
    int num_blocks = 0;
    ...
    while(!file_complete)
    {
        ...
        while (decoded_info.first > 0)
        {
			...
			if (select_ret > 0) {
            	...
            	if(!first_packet_rx) {
	                ...
	                if(nc_vector.size() < N_TB_SIZE) {
	                	...
	                	if (verb) {std::cout << "Not yet N_TB_SIZE packets, packets_needed " << N_TB_SIZE - nc_vector.size() << " ack_flag " << ack_flag <<  "\n";}
            ...
            if(nc_vector.size() >= N_TB_SIZE)
            {
                ...
                total_time_decoding_attempt += elapsed_seconds_packet_decoder.count(); // count all decoding attempts
                num_decoding_attempt++;
