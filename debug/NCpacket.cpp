...
NCpacket::NCpacket(vector<char*> data)
{
    ...
    cout<<"ok3.3\n";
    memcpy(packet.payload, tmp, PAYLOAD_SIZE); //size: sizeof(temp) instead of PAYLOAD_SIZE
                                              //MP: dovrebbero essere uguali!!
    cout<<"ok3.4\n";
    packet.header=binary_to_unsigned_int(encoding_vector);
    cout<<":::::::::::::packet EV is"<<endl;
     write_matrix(encoding_vector,0);
    printf(">>>>>>tmp is payload is : %s________\n",tmp);
    printf(">>>>>>packet payload is : %s________\n",packet.payload);
